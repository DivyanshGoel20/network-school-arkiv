...
export default function SpectatorDashboard() {
  ...
  const [selectedAgentTab, setSelectedAgentTab] = useState<SuspectId>("APEX");
  ...
  if (!mounted) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center p-12 text-center border border-zinc-850 bg-zinc-950/20 rounded-3xl min-h-[550px]">
        <span className="text-[20px] font-mono text-zinc-500 uppercase tracking-widest leading-none">
      ...
      <div className="flex flex-col flex-1 bg-[#121318] border border-zinc-800 rounded-2xl overflow-hidden shadow-2xl min-h-[550px] relative">
        <div className="flex bg-[#0d0e12] border-b border-zinc-800 p-1.5 gap-1.5 z-10 relative">
          <button
            ...
            className={`flex-1 py-3 flex items-center justify-center gap-2 text-[20px] font-bold font-mono uppercase tracking-widest rounded-lg transition-all duration-300 folder-tab ${activeTab === "notebooks"
          ...
          <button
            ...
            className={`flex-1 py-3 flex items-center justify-center gap-2 text-[20px] font-bold font-mono uppercase tracking-widest rounded-lg transition-all duration-300 folder-tab ${activeTab === "timeline"
              ? "bg-[#ebdcb9] border border-[#b89255] text-zinc-950 shadow-inner folder-tab-active"
              : "text-zinc-500 hover:text-zinc-300 hover:bg-zinc-900/40"
              }`}
          >
              ...
              <AnimatePresence mode="wait">
                {activeMonologue ? (
                  ...
                  >
                    ...
                    />
                    <div className="flex-1 text-[22px] font-mono leading-relaxed text-zinc-300">
                      <span className="font-bold text-[#b89255] block uppercase tracking-widest text-[18px] mb-1">
                  ...
                  <div key="waiting" className="flex items-start gap-3 p-4 bg-zinc-900/40 border border-zinc-800 rounded-xl">
                    ...
                    <div className="flex-1 text-[22px] font-mono leading-relaxed text-zinc-500 uppercase tracking-widest">
                ...
                {players.map((p) => {
                  ...
                  return (
                    <button
                      ...
                      className={`px-4 py-2 text-[20px] font-bold font-typewriter uppercase tracking-widest border transition-all duration-200 rounded-t-lg -mb-[1px] relative flex items-center gap-2 ${isSelected
                    ...
                    >
                      ...
                      <span>{p.name}</span>
                      {isActive && (
                        <span className="text-[14px] px-1.5 py-0.5 bg-cyan-100 text-cyan-800 font-bold rounded border border-cyan-800/40 leading-none">
                          Active
                        </span>
                      )}
                      {p.eliminated && (
                        <span className="text-[14px] px-1.5 py-0.5 bg-red-150 text-red-800 font-bold rounded border border-red-800/40 leading-none line-through">
                          Out
                        </span>
                      )}
                    </button>
                  ...
                  <div className="manila-dossier p-6 shadow-2xl relative border-2 border-[#b89255] rounded-b-xl rounded-tr-none animate-in fade-in duration-300">
                    ...
                    <div className="flex items-center justify-between border-b-2 border-[#1c1d22]/30 pb-3 mb-5">
                      ...
                      <div className="flex items-center gap-2">
                        {isActive && (
                          <span className="px-2.5 py-0.5 text-[15px] bg-cyan-100 text-cyan-850 border-2 border-cyan-850 font-bold rounded uppercase tracking-wider">
                            ACTIVE DETECTIVE
                          </span>
                        )}
                        {p.eliminated && (
                          <span className="px-2.5 py-0.5 text-[16px] bg-red-100 text-red-850 border-2 border-red-850 font-bold rounded uppercase tracking-wider">
                            ELIMINATED
                          </span>
                        )}
                      </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-[#1c1d22] font-typewriter">
                      ...
                      <div className="flex flex-col gap-3 min-w-0">
                        <h5 className="text-[18px] text-[#1c1d22]/65 font-black uppercase tracking-widest border-b border-[#1c1d22]/20 pb-1.5 mb-1 flex items-center justify-between">
                          <span>Suspects</span>
                          <span className="text-[16px] font-normal">Ink Record</span>
                        </h5>
                        <div className="flex flex-col gap-1.5">
                          {SUSPECTS.map((s) => {
                            ...
                            return (
                              ...
                              >
                                <span className={`text-[15px] font-bold min-w-0 truncate pr-1 ${status === "ELIMINATED" ? "line-through text-[#1c1d22]/40 font-normal" : "text-[#1c1d22]"}`}>
                                  {s.name}
                                </span>
                  ...
                  >
                    ...
                    <div className="flex-1 text-xs">
                      <div className="flex items-center justify-between mb-1.5">
                        <span style={{ color }} className="font-bold font-mono tracking-wider text-[22px]">
                          {playerState?.name || "Aether Manor"}
                        </span>
                        <span className="text-[16px] text-zinc-500 font-mono">
                          Turn {log.turn} • {logTime}
                        </span>
                      </div>
                      ...
                      {log.txHash && (
                        <div className="mt-2 flex items-center">
                          <a
                            ...
                            className="inline-flex items-center gap-2 px-2.5 py-1 bg-zinc-950 border border-zinc-800 hover:border-[#b89255] hover:text-[#b89255] text-zinc-400 text-[16px] font-bold font-mono rounded uppercase tracking-wider transition-all duration-200"
                          >
...
function NotebookBadge({ status }: { status: NotebookStatus }) {
  if (status === "HELD_BY_ME") {
    return (
      <span className="px-2 py-0.5 rounded border-2 font-bold text-[12px] tracking-wider uppercase leading-none shrink-0 bg-emerald-50 border-emerald-800 text-emerald-850 font-typewriter">
        MY HAND
      </span>
    );
  }
  if (status === "ELIMINATED") {
    return (
      <span className="px-2 py-0.5 rounded border-2 font-bold text-[12px] tracking-wider uppercase leading-none shrink-0 bg-zinc-100 border-zinc-400 text-zinc-550 font-typewriter line-through decoration-[#b91c1c] decoration-2">
        RULED OUT
      </span>
    );
  }
  if (status === "HELD_BY_OTHER") {
    return (
      <span className="px-2 py-0.5 rounded border-2 font-bold text-[12px] tracking-wider uppercase leading-none shrink-0 bg-amber-50 border-amber-800 text-amber-850 font-typewriter">
        DISPROVED
      </span>
    );
  }
  // Possible
  return (
    <span className="px-2 py-0.5 rounded border-2 font-bold text-[12px] tracking-wider uppercase leading-none shrink-0 bg-zinc-50 border-zinc-300 text-zinc-500 font-typewriter">
      POSSIBLE
    </span>
  );
}
